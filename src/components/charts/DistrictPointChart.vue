...
let sum_per_district_highest = computed(() => {
	...
	districts.forEach((d) => {
		...
		for (const val of Object.values(districtData.value)) {
			if (
				activeData.value[val.id] &&
				props.series[val.id].type === "normal"
			)
				sum += val.data[d];
...
let sum = computed(() => {
	let output = 0;
	for (const val of Object.values(districtData.value)) {
		if (activeData.value[val.id] && props.series[val.id].type === "normal")
			output += val.data.sum;
	}
	return output;
});
...
function toggleActiveData(i) {
	...
	if (i < 3) handleDataSelection(null);
}
...
function handleDataSelection(index) {
	...
	if (index === null) {
		mapStore.addLayerFilter(
			...
			[
				props.chart_config.map_filter[1][selectedIndex.value],
				props.chart_config.map_filter[3].filter(
					(d, i) => activeData.value[i]
				),
			]
		);
	} else {
	...
	<div v-if="activeChart === 'DistrictPointChart'" class="districtchart">
		<div class="districtchart-title">
			...
			<div class="districtchart-title-legend">
				<p>多</p>
				<div :style="{ backgroundColor: districtColor }"></div>
				<p>少</p>
			</div>
			<div
