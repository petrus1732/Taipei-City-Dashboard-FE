...
const activeData = ref(props.series.slice().fill(true));
const focusedData = ref(null);

const coloredPoints = [];
props.series.forEach((serie, i) => {
	coloredPoints.push([]);
	serie.data.points.forEach((p) => {
...
const activeDataPoints = computed(() => {
	let output = [];
	coloredPoints.forEach((d, i) => {
		if (activeData.value[i]) output = output.concat(d);
	});
	return output;
});

const districts = ["北投區", "士林區", "內湖區", "南港區", "松山區", "信義區", "中山區", "大同區", "中正區", "萬華區", "大安區", "文山區"]

//convert coordinate to svg
const svgMinX = 1.7;
const svgMaxX = 411.8;
const svgMinY = 1.2;
const svgMaxY = 548.6;
const crdMinX = 121.45715;
const crdMaxX = 121.66594;
const crdMinY = 24.9605;
const crdMaxY = 25.21379;

const scaleX = (svgMaxX - svgMinX) / (crdMaxX - crdMinX);
const scaleY = (svgMaxY - svgMinY) / (crdMaxY - crdMinY);

const toSvg = {
	lng: (x) =>  (x - crdMinX) * scaleX + svgMinX,
	lat: (y) => (crdMaxY - y) * scaleY + svgMinY
};
...
const districtData = computed(() => {
	...
	props.series.forEach((serie, i) => {
		...
		highest = Object.values(name).sort(function (a, b) {
			return b - a;
		})[0];
		sum = Object.values(name).reduce(
...
let sum_per_district_highest = computed(() => {
	let output = {};
	let highest = 0
	districts.forEach((d) => {
		let sum = 0;
		for (const val of Object.values(districtData.value)) {
			if (activeData.value[val.id] && props.series[val.id].type === 'normal') sum += val.data[d];
			highest = highest > val.data[d]? highest : val.data[d];
		}
		output[d] = sum;
	})
...
let sum_per_district = computed(() => sum_per_district_highest.value[0]);
let highest = computed(() => sum_per_district_highest.value[1]);

let sum = computed(() => {
	let output = 0;
	for (const val of Object.values(districtData.value)) {
		if (activeData.value[val.id] && props.series[val.id].type === 'normal') output += val.data.sum
	}		
	return output;
})
...
function toggleActiveData(i) {
	activeData.value[i] = !activeData.value[i]
}

function toggleFocusData(i) {
	focusedData.value = i
}

function toggleFocusToNull() {
	focusedData.value = null
}
...
<template>
	<div v-if="activeChart === 'DistrictPointChart'" class="districtchart">
		<div class="districtchart-title">
			...
			<div
				class="districtchart-title-legends"
				v-for="(value, index) in props.series"
				:key="index"
				@click="toggleActiveData(index)"
				@mouseenter="toggleFocusData(index)"
				@mouseleave="toggleFocusToNull()"
				:style="{
					opacity: activeData[index]? 1:0.5
				}"
			>
				<span
		...
		<div class="districtchart-chart">
			...
			<svg viewBox="0 0 413 550" xmlns="http://www.w3.org/2000/svg">
				<g>
					<path
						...
						:fill-opacity="
							sum_per_district['北投區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['士林區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['內湖區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['中山區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['大同區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['中正區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['萬華區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['大安區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['信義區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['松山區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['南港區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					<path
						...
						:fill-opacity="
							sum_per_district['文山區'] / highest * 0.9 + 0.1
						"
						:fill="districtColor"
					...
					/>
					<circle
						v-for="(p, index) in activeDataPoints"
						:key="index"
						:cx="toSvg.lng(p.lng)"
						:cy="toSvg.lat(p.lat)"
						:fill="p.color"
						r="4"
						stroke-width="0"
						:data-name="p.district"
						:class="{
							'active-district':
								targetDistrict === p.district ||
								selectedIndex === index
						}"
						:opacity="(targetDistrict === p.district || targetDistrict === null) && focusedData !== p.data_id? 1:0.2"
						@mouseenter="toggleActive"
						@mousemove="updateMouseLocation"
						@mouseleave="toggleActiveToNull"
			...
			<Teleport to="body">
				...
				>
					...
					<span
						>{{ sum_per_district[targetDistrict] }}
